...
bucket_name = '436-cloud-comptuting'
...
def create_user_submit():
    ...
    if email == '' or (password == '') or (username == ''):
        ...
        if len(response['Items']) == 1:           #user already exists if an Item is returned
            print('user exists!')
            flash('User already exists!')
            return render_template('create_user.html')
        else:   #user doesn't exist, all fields were filled, and email format is correct
            if file_to_upload and check_file_type(file_to_upload.filename):  #valid file type
                file_name = secure_filename(file_to_upload.filename)
                print(file_name)
                response = s3_client.put_object(ACL='public-read', Body=file_to_upload, Bucket=bucket_name,
                                                Key=file_name)
                print("file uploaded!")
